...
5. **Обновление версии:**
   ...
   ```bash
   make update_version vX.Y.Z
   ```

   Аргумент `vX.Y.Z` можно заменить на любую возможную версию, см. [3. Разработка proto, go и dart.](#3-разработка-proto-go-и-dart-git-теги-для-версионирования)

   Например:

   ```bash
   make update_version v1.0.0
   ```
