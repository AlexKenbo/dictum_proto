...
Makefile предоставляет несколько удобных команд для управления проектом, такими как сборка, обновление версии и генерация кода. Исходные команды `docker` можно также найти ниже в разделе [Ручная генерация](#ручная-генерация).
...
### 1. Компиляция
   ...
   > [!WARNING] Примечание
   > Если после генерации на Linux/MacOS нет прав на редактирования сгенерированных файлов, нужно выдать права на проблемную директорию:
   >
   >```bash
   >   sudo chmod -R 777 $(pwd)/<directory>
   >```
